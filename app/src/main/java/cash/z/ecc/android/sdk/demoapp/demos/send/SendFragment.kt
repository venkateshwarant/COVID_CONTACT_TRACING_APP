...
class SendFragment : BaseDemoFragment<FragmentSendBinding>() {
    ...
    private fun setup() {
        ...
        var seedPhrase = sharedViewModel.seedPhraseBytes.value!!
        // Use a BIP-39 library to convert a seed phrase into a byte array. Most wallets already
        // have the seed stored
      //  val seed = Mnemonics.MnemonicCode(seedPhrase).toSeed()
        App.instance.defaultConfig.let { config ->
            Initializer(App.instance) {
                it.importWallet(seedPhrase, config.birthdayHeight)
                it.server(config.host, config.port)
            }.let{ initializer ->
                synchronizer = Synchronizer(initializer)
            }
            spendingKey = DerivationTool.deriveSpendingKeys(seedPhrase).first()
        }
    }
    ...
    private fun initSendUi() {
        ...
        binding.createAddress.setOnClickListener(::onCreateAddress)
    }
    ...
    private fun onSend(unused: View) {
        ...
        //val toAddress = toUser.getSaplingKeyAddress().trim()
        val toAddress= binding.toZAddress.text.toString()
       ...
       // var seedPhrase = sharedViewModel.seedPhrase.value
        ...
        synchronizer.sendToAddress(
            ...
            amount,
            toAddress,
            "Funds from Venkat new"
        ).collectWith(lifecycleScope, ::onPendingTxUpdated)
        mainActivity()?.hideKeyboard()
    }

    private fun onCreateAddress(unused: View) {
        fromUser = User()
        val  seed = sharedViewModel.seedPhraseBytes.value
        ...
        binding.fromZAddress.setText(fromUser.getSaplingKeyAddress())
        binding.fromViewingAddress.setText(fromUser.getViewingKey())
        binding.fromSpendingKey.setText(fromUser.getSpendingKey())

        DerivationTool.deriveShieldedAddress(DerivationTool.deriveViewingKeys(fromUser.getSeedByteArr(),10).get(2))
        //binding.toZAddress.setText(toUser.getSaplingKeyAddress())
